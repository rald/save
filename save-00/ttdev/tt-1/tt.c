...
Player *Player_New(char *nick,int pts) {
...
void Player_Add(Player ***p,int *pn,Player *player) {
	(*p)=realloc((*p),sizeof(**p)*((*pn)+1));
	(*p)[(*pn)++]=player;
}
...
char *clue(char *w) {
	char *c=calloc(strlen(w)+1,sizeof(*c));
	int i;
	for(i=0;i<strlen(w);i++) {
		c[i]='*';
	}
